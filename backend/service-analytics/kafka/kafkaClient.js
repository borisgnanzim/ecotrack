// kafka/kafkaClient.js - Client Kafka pour service-analytics
const { Kafka, logLevel } = require('kafkajs');

const kafkaClient = new Kafka({
  clientId: `${process.env.SERVICE_NAME || 'service-analytics'}-${Date.now()}`,
...
const getProducer = async () => {
  if (!producer) {
    ...
    console.log('✅ Kafka Producer connecté (service-analytics)');
  }
  return producer;
};
 ...
 */
const ensureTopicsExist = async (topicsToCreate) => {
  try {
...
const disconnectKafka = async () => {
  try {
    if (producer) {
      ...
      console.log('✅ Kafka Producer déconnecté (service-analytics)');
    }
