// kafka/kafkaClient.js - Client Kafka pour service-routes
const { Kafka, logLevel } = require('kafkajs');

const kafkaClient = new Kafka({
  clientId: `${process.env.SERVICE_NAME || 'service-routes'}-${Date.now()}`,
...
const getProducer = async () => {
  if (!producer) {
    ...
    console.log('✅ Kafka Producer connecté (service-routes)');
  }
  return producer;
};
...
const disconnectKafka = async () => {
  try {
    if (producer) {
      ...
      console.log('✅ Kafka Producer déconnecté (service-routes)');
    }
