// kafka/kafkaClient.js - Client Kafka pour service-users
const { Kafka, logLevel } = require('kafkajs');

const kafkaClient = new Kafka({
  clientId: `${process.env.SERVICE_NAME || 'service-users'}-${Date.now()}`,
...
const getProducer = async () => {
  if (!producer) {
    ...
    console.log('✅ Kafka Producer connecté (service-users)');
  }
  return producer;
};
...
const disconnectKafka = async () => {
  try {
    if (producer) {
      ...
      console.log('✅ Kafka Producer déconnecté (service-users)');
    }
